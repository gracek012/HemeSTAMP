...
THRESHOLD_DEPENDENT = ['accuracy_score', 'recall_score', 'precision_score', 'get_npv_score']
...
class BinaryEvaluator:
    ...
    def __init__(self, outdir):
        ...
        self.metrics = {
            ...
            'Negative Predictive Value': self.get_npv_score,
            'NPV_0_thresh': self.get_npv_score,
            'NPV_10_thresh': self.get_npv_score,
            'NPV_30_thresh': self.get_npv_score,
            'NPV_50_thresh': self.get_npv_score,
            'NPV_70_thresh': self.get_npv_score,
            'NPV_90_thresh': self.get_npv_score,
            'NPV_100_thresh': self.get_npv_score
        }
    ...
    def get_npv_score(self, labels, predictions):
    ...
    def plot_scatter(self, results):
        ...
        for metric in results:
            if "NPV" in metric:
                parsed = parse.parse("{:.2f} [{:.2f}, {:.2f}]", results[metric])
                print(parsed)
                point.append(parsed[0])
                lower.append(parsed[1])
                upper.append(parsed[2])
        plt.scatter(lower, upper)
        plt.errorbar(lower, upper, yerr=point, fmt="o")
        plt.show()
    ...
    def bootstrap_metric(self, labels, predictions, metrics, iters=1000,
            ...
            for m in metrics:
                if metrics[m].__name__ in THRESHOLD_DEPENDENT:
                    if m == 'Specificity':
                        ...
                        new_threshold = float(m.split("_")[1])/100
                        new_predicted_labels = np.asarray([1 if p > new_threshold else 0 for p in predictions])
                        new_p_b_l = new_predicted_labels[inds_b]
                        values.setdefault(m, []).append(metrics[m](l_b, new_p_b_l))
                    else:
        ...
        for m in metrics:
            if metrics[m].__name__ in THRESHOLD_DEPENDENT:
                if m == 'Specificity':
                    ...
                    new_threshold = float(m.split("_")[1]) / 100
                    new_predicted_labels = np.asarray([1 if p > new_threshold else 0 for p in predictions])
                    values.setdefault(m, []).append(metrics[m](labels, new_predicted_labels))
                    print(f"threshold {new_threshold}: {metrics[m](labels, new_predicted_labels)}")
                else:
                    values.setdefault(m, []).append(
        ...
        for v in values:
            if "NPV" in v:
                p = 4
            mean = '{:.2f}'.format(round(np.mean(values[v]), p))
            upper = '{:.2f}'.format(round(np.percentile(values[v], 97.5), p))
            lower = '{:.2f}'.format(round(np.percentile(values[v], 2.5), p))
